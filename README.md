...
- Actions are packets of information that contain an action type and some data that is associated with the action. Reducers calculate the new version of the state based on the current state and the action given. Store is where the state of the application is stored. Its known as the single source of truth because  everything that changes within our app happens in the store. 

- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?

- Component state is mutable and Application state/store is immutable. When having multiple components with multiple states it would be a good idea to use store/application state because your state is being mananged in one place instead of multiple files. Also it helps minimize errors. 

- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

- Redux-thunk is middleware that allows us to intercept the normal redux-flow that allows us to do something before actions make it to the reducer. So when an action creator is called, redux-thunk will intercept and look at what was and  if an action was returned then it will forward the action through to the reducer. If a thunk was returned (basically a function returned from a function), then it will invoke that function and pass in the dispatch function as an argument to it.
